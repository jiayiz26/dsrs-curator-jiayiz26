...
from pipeline.source import build_source
from pipeline.structure import build_parquet
...
def run(user_agent: str, output: Path) -> None:
    ...
    """
    # Chapter 1 is intentionally isolated so its network/caching behavior can be
    # tested independently; Chapter 3 consumes the paired XML artifacts.
    filings = build_source(load_filers(), output, ROOT / ".cache", user_agent)
    filing_count, holding_count = build_parquet(output)
    print(f"processed {len(filings)} source filings; wrote {filing_count} filing rows and {holding_count} holding rows", file=sys.stderr)
